...
export class StateMachineDetector {
  ...
  private findStateTransitions(
    lines: string[],
    varName: string
  ): Array<{ from: number; to: number; trigger: string; line: number }> {
    const transitions: Array<{ from: number; to: number; trigger: string; line: number }> = [];
    const checkPattern = new RegExp(
      `when\\s*\\(.*${varName}\\s*==\\s*(\\d+).*\\)\\s*\\[(\\w+)\\]`,
      'i'
    );
  ...
  private inferStateName(state: number, varName: string, lines: string[]): string {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (statePattern.test(lines[i])) {
        ...
        const comment =
          lines[i].match(/#\s*(.+)/) || (i > 0 ? lines[i - 1].match(/#\s*(.+)/) : null);
        if (comment) {
...
export class ResourceFlowAnalyzer {
  ...
  public analyzeResourceFlow(scriptContent: string): Map<string, ResourceFlow> {
    ...
    const resources: Array<'crystals' | 'ore' | 'studs' | 'air'> = [
      'crystals',
      'ore',
      'studs',
      'air',
  ...
  public getRecommendations(metrics: PerformanceMetrics): string[] {
    ...
    if (metrics.conditionComplexity > 50) {
      recommendations.push(
        'Complex conditions may impact performance - consider simplifying or using state machines'
      );
    }
    ...
    if (metrics.eventCount > 100) {
      recommendations.push(
        'Large number of events - consider using event chains to reduce duplication'
      );
    }
...
export class CircularDependencyDetector {
  ...
  private dfs(event: string, visited: Set<string>, path: string[]): void {
    if (path.includes(event)) {
      ...
      const alreadyReported = this.circularDeps.some(
        dep => dep.events.sort().join('-') === cycleKey
      );
