...
export const EVENT_BLOCKS = {
  ...
  EventRandomSpawnSetup: {
    params: ['row', 'col', 'type', 'maxTime', 'minTime', 'maxWave', 'minWave', 'maxSpawn', 'minSpawn'],
...
export class VisualBlocksParser {
  ...
  constructor(private content: string, private startLine: number = 0) {}
  ...
  private parseBlock(match: RegExpMatchArray, lineIndex: number): void {
    ...
    const blockDef = blockDefs[blockType as keyof typeof blockDefs] as { params: string[]; description: string } | undefined;
    
    if (!blockDef) {
      this.errors.push(`Unknown block type '${blockType}' at line ${this.startLine + lineIndex + 1}`);
      return;
    }
    ...
    // Basic validation
    if (params.length < 2) {
      this.errors.push(`Block ${id} missing required row,col parameters at line ${this.startLine + lineIndex + 1}`);
      return;
    }
    ...
    if (isNaN(row) || isNaN(col)) {
      this.errors.push(`Block ${id} has invalid row,col coordinates at line ${this.startLine + lineIndex + 1}`);
      return;
    }
    ...
    for (let i = 2; i < expectedParams.length && i < params.length; i++) {
      ...
      } else if (paramName === 'cooldown' || paramName === 'delay' || paramName.includes('Time') || paramName === 'radius') {
        // Numeric parameters
  ...
  private parseWire(match: RegExpMatchArray, lineIndex: number): void {
    ...
    if (!fromBlock) {
      this.errors.push(`Wire references unknown block ${from} at line ${this.startLine + lineIndex + 1}`);
    }
    if (!toBlock) {
      this.errors.push(`Wire references unknown block ${to} at line ${this.startLine + lineIndex + 1}`);
    }
    ...
    switch (wireType) {
      ...
      case '~':
        ...
        if (fromBlock && !fromBlock.name.includes('Emerge')) {
          this.errors.push(`Backup wire (~) can only be used from emerge blocks at line ${this.startLine + lineIndex + 1}`);
        }
        break;
      case '?':
        type = 'random';
  ...
  public validate(): string[] {
    ...
    for (const [source, targets] of randomSources) {
      if (targets.length < 2) {
        validationErrors.push(`Block ${source} has random wire(s) but only ${targets.length} target(s) - random wires work best with multiple targets`);
      }
    }
